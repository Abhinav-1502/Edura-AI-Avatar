...
class ApiClientService {
    ...
    async getHeyGenToken(): Promise<string> {
        ...
        if (!response.ok) throw new Error('Failed to get LiveAvatar session token');
    ...
    async getTtsAudio(text: string, signal?: AbortSignal): Promise<string> {
        const response = await fetch(`${this.BASE_URL}/api/heygen/tts`, {
            method: 'POST',
